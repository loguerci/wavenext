...
class Generator(nn.Module):
    def __init__(self, in_channels: int, dim: int, shift_dim: int, inter_channels: int, num_blocks: int):
        ...
        self.blocks = nn.ModuleList([ConvNeXt(dim, inter_channels) for _ in range(num_blocks)])
...
if "__main__" == __name__: # run python -m wavenext.models.generator 
    ...
    model = Generator(in_channels=80, dim=512, shift_dim=256, inter_channels=512*3, num_blocks=4)
    out = model(mel)
